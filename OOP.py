...
class PriorityQueue:
    ...
    def insert(self, e):
        ...
        while i > 0 and self.heap[self._parent(i)] > self.heap[i]: # While parent is bigger
            p = self._parent(i)
            self.heap[i], self.heap[p] = self.heap[p], self.heap[i] # Switch parent and child
    ...
    def extractMin(self):
        ...
        self.heap[0] = self.heap.pop() # We use the last element to become the root, O(1)
    ...
    def _min_heapify(self, i):
        ...
        if left < len(self.heap) and self.heap[left] < self.heap[smallest]: # Check if Index exists and If left child smaller than parent 
        ...
        if smallest != i: # Here if the smallest have been changed to child's index then:
            self.heap[i], self.heap[smallest] = self.heap[smallest], self.heap[i] # Switch element and child
